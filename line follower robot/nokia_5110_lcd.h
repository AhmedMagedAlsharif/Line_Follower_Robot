...
#define DC 1
#define RST 0
#define ssenable PORTB&=~(1<<SS)
#define ssdisable PORTB|=(1<<SS)
...
void print_char( char data)
	{
		...
		for (int index=0; index<5; index++)
			{
		spi_write(ASCII[data - 0x20][index]<<1);	/* send the data on data register */
			}
		spi_write(0x00);
		ssdisable;
}
...
void print_image(const unsigned char *image_data)		/* clear the Display */
{
	ssenable;
	PORTB |= (1<<DC);
	for (int k=0; k<=503; k++)
...
void print_number(int16_t num)
{
	...
	do 
	{
		numcache[index]=num % 10;
		num/=10;
		index++;
	} while (num);
	while (index)
	{
		print_char(numcache[index-1]+48);
		index--;
	}
}
